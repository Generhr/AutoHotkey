...
OrderedObject(parameters*) {
}

Print(input) {
    switch (input.__Class) {
        case "__Array": {
            if (s := input.Length) {
                for i, v in (input, r := "[") {
...
Class __Array {
    ...
    __Call(key, skipEmpty := 0) {
        switch (key) {

            ;* array.Count(skipEmpty)
            case "Count": {
                return (this.Count[skipEmpty])
            }
    ...
    __Get(index) {
        ...
        if (index < 0 && index.IsInteger()) {
            index := Max(0, index + this.Length)
    ...
    Count[skipEmpty := 0] {
        ...
        Get {
            ...
            return (ObjCount(this))  ;! return (NumGet(&this + A_PtrSize*4))
        }
    }
    ...
    Length[] {
        ...
        Set {
            if (!value.IsInteger() || value < 0) {
                throw (Exception("IndexError", -1, Format("""{}"" is invalid. This property may only be assigned a non negative integer.", value)))
            }
    ...
    Sample(n := 0) {
        if (!s := this.Length) {
            throw (Exception("LengthException.", -1, Format(".Sample({}) was called on an empty object.", n)))
        }
    ...
    Swap(index1, index2) {
        if (!s := Round((m := this.MaxIndex()) + 1) || s < 2) {
            throw (Exception("LengthException.", -1, Format(".Swap({}) was called on an empty object.", [index1, index2].Join(", "))))
        }

        if (index1 >= 0 && index1 <= m && index2 >= 0 && index2 <= m) {  ;~ No error handling. ** Register negative lookups. **
    ...
    Every(callback) {
        if (!this.Length) {
            throw (Exception("LengthException.", -1, Format(".Every({}) was called on an empty object.", callback)))
        }

        if (!IsFunc(callback)) {
            throw (Exception("TypeError.", -1, Format(".Every({}) was not passed a function.", callback)))
        }
    ...
    Filter(callback) {
        if (!this.Length) {
            throw (Exception("LengthException.", -1, Format(".Filter({}) was called on an empty object.", callback)))
        }

        if (!IsFunc(callback)) {
            throw (Exception("TypeError.", -1, Format(".Filter({}) was not passed a function.", callback)))
        }
    ...
    Find(callback) {
        if (!this.Length) {
            throw (Exception("LengthException.", -1, Format(".Find({}) was called on an empty object.", callback)))
        }

        if (!IsFunc(callback)) {
            throw (Exception("TypeError.", -1, Format(".Find({}) was not passed a function.", callback)))
        }
    ...
    FindIndex(callback) {
        if (!IsFunc(callback)) {
            throw (Exception("TypeError.", -1, Format(".FindIndex({}) was not passed a function.", callback)))
        }
    ...
    ForEach(callback) {
        if (!this.Length) {
            throw (Exception("LengthException.", -1, Format(".ForEach({}) was called on an empty object.", callback)))
        }

        if (!IsFunc(callback)) {
            throw (Exception("TypeError.", -1, Format(".ForEach({}) was not passed a function.", callback)))
        }
    ...
    Map(callback) {
        if (!this.Length) {
            throw (Exception("LengthException.", -1, Format(".Map({}) was called on an empty object.", callback)))
        }

        if (!IsFunc(callback)) {
            throw (Exception("TypeError.", -1, Format(".Map({}) was not passed a function.", callback)))
        }
    ...
    ;* array.Reduce(Func("FunctionName"))
    ;* Description:
        ;* Executes a callback function on each element of the array, resulting in a single output value.
    Reduce(callback, initialValue := "") {
        if (!Round((m := this.MaxIndex()) + 1)) {
            throw (Exception("LengthException.", -1, Format(".Reduce({}) was called on an empty object.", [callback, initialValue].Join(", "))))
        }

        if (!IsFunc(callback)) {
            throw (Exception("TypeError.", -1, Format(".Reduce({}) was not passed a function.", [callback, initialValue].Join(", "))))
        }
        ...
        while (index <= m) {
            initialValue := callback.Call(initialValue, this[index], index++, this)
        }

        return (initialValue)
    }

    ;* array.ReduceRight(Func("FunctionName"))
    ;* Description:
        ;* Executes a callback function on each element of the array from right to left, resulting in a single output value.
    ReduceRight(callback, initialValue := "") {
        if (!Round((m := this.MaxIndex()) + 1)) {
            throw (Exception("LengthException.", -1, Format(".ReduceRight({}) was called on an empty object.", [callback, initialValue].Join(", "))))
        }

        if (!IsFunc(callback)) {
            throw (Exception("TypeError.", -1, Format(".ReduceRight({}) was not passed a function.", [callback, initialValue].Join(", "))))
        }
        ...
        while (--m >= 0) {
            initialValue := callback.Call(initialValue, this[m], m, this)
        }
    ...
    Slice(start := 0, end := "") {
        if (!(s := this.Length)) {
            throw (Exception("LengthException.", -1, Format(".Slice({}) was called on an empty object.", [start, end].Join(", "))))
        }
    ...
    Some(callback) {
        if (!IsFunc(callback)) {
            throw (Exception("TypeError.", -1, Format(".Some({}) was not passed a function.", callback)))
        }
...
Class __String {
    ...
    ;* "String".IsString()
    IsString() {
        if this is Float  ;* Floats are already in the string buffer so I'm siding with not string since I can't think of a way to differentiate between `1.0` and `"1.0"`.
            return (0)
