...
Assert.IsEqual(Matrix3(3, 2, 1, 6, 3, 3, 1, 4, 5).Multiply(Matrix3(3, 2, 1, 6, 3, 3, 1, 4, 5)), [22, 16, 14, 39, 33, 30, 32, 34, 38])
Assert.IsEqual(Matrix3(0.5, 0, 0, 4.5, 0.5, -2.25, -1, 0, 0.5).Multiply(Matrix3(1, 1, 0, 0, 0.5, 0, 0, 1, 1)), [0.5, 0.5, 0, 4.5, 2.5, -2.25, -1, -0.5, 0.5])
...
Assert.IsEqual(Matrix3(0, 0, 0, 0, 0, 0, 0, 0, 0).Invert(), [0, 0, 0, 0, 0, 0, 0, 0, 0])
...
;Console.Log(RotationMatrix(Math.ToRadians(0), Math.ToRadians(0), Math.ToRadians(90)).Print())


;Assert.IsEqual(r := RotationMatrix(-2.6337, -0.47158, -1.2795), [0.25584740067661427, -0.77350869250143017, 0.57985033430338551, -0.85332215959849234, -0.46257452803122612, -0.24055373194954496, 0.45429439739576927, -0.43325409246932256, -0.77840059856952748])
;Assert.IsEqual(r.Roll, -2.6337)
;Assert.IsEqual(r.Pitch, -0.47158)
;Assert.IsEqual(r.Yaw, -1.2795)

Assert.IsEqual(r := RotationMatrix(-(3*Math.Pi/4), 3*Math.Pi/4, -(3*Math.Pi/4)), [0.49999999999999989, -0.14644660940672621, 0.85355339059327384, 0.5, 0.85355339059327373, -0.14644660940672621, -0.70710678118654757, 0.5, 0.49999999999999989])
Assert.IsEqual(r.Roll, -(3*Math.Pi/4))
Assert.IsEqual(r.Pitch, 3*Math.Pi/4)
Assert.IsEqual(r.Yaw, -(3*Math.Pi/4))
