...
for v in ["", "[&1] Edit", "[&2] Open Script Folder", "[&3] Window Spy", "[&4] List Lines", "[&5] List Vars", "[&6] List Hotkeys", "[&7] KeyHistory", "", "[&8] Pause", "[&9] Suspend", "", "[10] Restore All", "[11] Reload", "[12] Exit", ""] {
    if (v) {
        ...
        TrayMenu.Add()  ;* Creates a separator line.
    }
}

CaseMenu := Menu()
for v in ["", "[&0] Check spelling", "", "[&1] lowercase", "[&2] UPPERCASE", "[&3] Sentence case", "[&4] Title Case", "", "[&5] iNVERSE", "[&6] esreveR", ""] {
    if (v) {
...
for k, v in Map("Browser", [["ahk_class Chrome_WidgetWin_1 ahk_exe chrome.exe"], ["ahk_class Chrome_WidgetWin_1 ahk_exe msedge.exe"]]  ;? [["Title ahk_class ClassName ahk_exe ProcessName", "ExcludeTitle"], ...]
...
;q:: {
;   MsgBox(DllCall("random\boolean", "Double", 0.5))
;}
...
$c:: {
    if (!KeyWait("c", "T0.25")) {
        ...
        if (text := Copy()) {
            comment := (extensionLookup.Has(extension := RegExReplace(WinGetTitle("A"), "iS).*\.([a-z]+).*", "$1"))) ? (extensionLookup[extension]) : ("//")
...
$F9:: {
    loop Files ("C:\Users\Onimuru\OneDrive\__User\Pictures\__Rename\*.*") {
        loop (10 + 5*((extension := RegExReplace(A_LoopFileName, "i).*(\.\w+).*", "$1")) ~= "gif|mov|mp4|webm" != 0)) {
            extension := Math.Random(0, 9) . extension
        }
...
#HotIf (WinActive("Memes ahk_exe explorer.exe"))

$F9:: {
    loop Files ("C:\Users\Onimuru\OneDrive\__User\Pictures\Memes\*.mp4") {
        if (!(SubStr(A_LoopFileName, -10, 6) ~= "\[\d\.\d\d\]")) {
            FileMove(A_LoopFilePath, A_LoopFileDir . "\" . RegExReplace(A_LoopFileName, "." . extension := RegExReplace(A_LoopFileName, "i).*(\.\w+).*", "$1")) . " [1.00]" . extension)

            if (debug) {
                Console.Log(Format("{} --> {}", A_LoopFileName, RegExReplace(A_LoopFileName, "." . extension := RegExReplace(A_LoopFileName, "i).*(\.\w+).*", "$1")) . " [1.00]" . extension))
            }
        }
    }
...
#HotIf (WinActive("ahk_class MediaPlayerClassicW ahk_exe mpc-hc64.exe") || WinActive("Memes ahk_exe explorer.exe"))
 ...
 */
BackSpace:: {  ;: https://trekhleb.medium.com/weighted-random-in-javascript-4748ab3a1500
    static memes := __Build()

    __Build() {
        memes := []

        loop Files ("C:\Users\Onimuru\OneDrive\__User\Pictures\\Memes\*.mp4") {
            memes.Push({Path: A_LoopFilePath, Weight: SubStr(A_LoopFilePath, -9, 4)})
        }
    ...
    loop (length) {
        ...
        try {
            cumulativeWeights.Push(memes[i].Weight + cumulativeWeights[i - 1])
        }
        catch {
            cumulativeWeights.Push(memes[i].Weight)
        }
    }
    ...
    maxCumulativeWeight := cumulativeWeights[-1]
    ...
    loop (length) {
        ...
        if (cumulativeWeights[i] >= randomNumber) {
            path := memes[i].Path
            weight := memes[i].Weight

            extension := RegExReplace(path, "i).*(\.\w+).*", "$1")
            newPath := RegExReplace(path, "\[" . weight . "\]" . extension) . "[" . (weight := Math.Round(weight - 0.1, 2)) . "]" . extension
            ...
            if (debug) {
                Console.Log(Format("{} -->`n{}", path, newPath))
            }

            memes[i].Weight := weight
...
MenuHandler(thisMenuItem := "", thisMenuItemPos := 0, thisMenu := TrayMenu) {
    switch (thisMenu.Handle) {
            ...
            switch (RegExReplace(thisMenuItem, "iS).*?([a-z])", "$1")) {
                ...
                default:
                    ...
                    if (!WinExist("ahk_id" . hWnd)) {
                        MsgBox(Format("There was an error showing this window ({}). Please contact your system administrator.", thisMenuItem))
                    }
        ...
        case CaseMenu.Handle:
            ...
            switch (RegExReplace(thisMenuItem, "iS).*?([a-z])", "$1")) {
                case "Checkspelling":
                    exec := shell.Exec(A_ComSpec " /C " . A_ScriptDir . '\..\tools\CheckSpelling.py "' . text . '"')
                ...
                case "TitleCase":
                    exec := shell.Exec(A_ComSpec " /C " . A_ScriptDir . '\..\tools\Capatalise.py "' . text . '"')
                    text := exec.StdOut.ReadAll()
                case "iNVERSE":
