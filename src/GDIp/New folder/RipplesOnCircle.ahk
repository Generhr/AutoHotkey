...
global circles := []
    , numberOfCirclesToCreate := 10, timeToRespawn := 10000

SetTimer(Create, timeToRespawn/numberOfCirclesToCreate)  ;* I tried to think of a way to interpolate over the sum of both interpolations but in the end I gave up and just opted to break the alpha of the interpolations based on the amount of time that should pass for both to complete.

Create(*) {
    circles.Push(Circle(270, 149, 0))

    if (circles.Length == numberOfCirclesToCreate) {
        SetTimer(Create, 0)
    }
}
...
/*
    ** Simple Interpolation: https://codeplea.com/simple-interpolation **
*/
...
Update(delta) {  ;* Simulates everything that is affected by time. It can be called zero or more times per frame depending on the frame rate.
    for circle in circles {
        circle.Alpha += delta/circle.Step

        circle.Posiiton := Vec2.Lerp(circle.Origin, circle.Target, circle.Alpha), circle.Radius := circle.Origin[2] + (circle.Target[2] - circle.Origin[2])*circle.Alpha
    }

    for index, circle in circles {
        switch (circle.Target[2]) {
            case 150:
                if (circle.Radius >= 150) {
                    circle.Origin := [circle.Posiiton[0], circle.Posiiton[1], circle.Radius], circle.Target := [80, 149, 0]
                        , circle.Alpha := 0, circle.Step *= 0.5
            }
            case 0:
                if (circle.Radius <= 0) {
                    static restore := (timeToRespawn*2/3)*0.001

                    circle.Radius := 0
                        , circle.Origin := [270, 149, 0], circle.Target := [0, 0, 150]

                        , circle.Alpha := 0, circle.Step := restore

                        Console.Log((time := GetTime()) - circle.Mark), circle.Mark := time
                }
        }
    }
}
...
Draw(interpolation := 0) {  ;* A function that draws things on the screen.
    for circle in circles {
        ...
        canvas.Graphics.DrawEllipse(pen[1], circle.Posiiton[0], circle.Posiiton[1], diameter, diameter)
    }
...
class Circle {
    ...
    __New(x, y, radius) {
        this.Posiiton := Vec2(x, y)
            , this.Radius := radius

        this.Target := [0, 0, 150], this.Origin := [270, 149, 0]
            , this.Alpha := Math.Map(radius, 0, 150, 0, 1.0), this.Step := (timeToRespawn*2/3)*0.001  ;* `timeToRespawn*2/3` milliseconds for alpha to reach 1 (this happens twice per cycle and the the second is sped up by a factor of 2).
