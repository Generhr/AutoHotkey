...
global canvas := LayeredWindow(A_ScreenWidth - (150*2 + 50 + 10), 50, 150*2, 150*2, "Canvas", ["CS_HREDRAW", "CS_VREDRAW"], WindowProc, 32512, false, ["WS_EX_LAYERED", "WS_EX_NOACTIVATE", "WS_EX_TOOLWINDOW", "WS_EX_TOPMOST"], ["WS_CLIPCHILDREN", "WS_POPUPWINDOW"], 0, "SW_SHOWNOACTIVATE", 0xFF, 0x000E200B, 7, 4)
    , brush := [GDIp.CreateSolidBrush(Color("Crimson")), GDIp.CreateSolidBrush(Color("AliceBlue"))]
...
loop (index := 0, 20) {
    points.Push(Point(270, index++*3 + 270))
}

global target := Vec2()
global targetRight := Vec2()
...
Update(delta) {  ;* Simulates everything that is affected by time. It can be called zero or more times per frame depending on the frame rate.
    static angle1 := Math.Pi*1.5, angle2 := Math.Pi*1.5, increment := Math.Pi/10

    angle1 += increment*delta, angle2 -= increment*delta

    global target := PointOnEllipse(149, 149, 150, angle1)
    global targetRight := PointOnEllipse(149, 149, 150, angle2)

    static currecnt := 60

    t := Mod(Math.Map(angle1, Math.Pi*1.5, Math.Pi*2.5, 0, 1.0), 1)

    for point in points {
;       if (angle > point.Trigger) {
            point.Position := Vec2.Lerp(point.Start, target, t)
;       }
    }
}
...
Draw(interpolation := 0) {  ;* A function that draws things on the screen.
    for point in points {
;       destination := PointOnEllipse(149, 149, 150, point.Angle)

        canvas.Graphics.FillEllipse(brush[0], point.Position[0] - 2, point.Position[1] - 2, 4, 4)
        canvas.Graphics.DrawLine(pen[1], point.Start[0], point.Start[1], target[0], target[1])

        canvas.Graphics.DrawLine(pen[1], point.Start[0], point.Start[1], targetRight[0], targetRight[1])
    }

    canvas.Graphics.FillEllipse(brush[1], target[0] - 2, target[1] - 2, 4, 4)
...
class Point {
    ...
    __New(angle, trigger) {
        theta := Math.ToRadians(angle)
            , this.Start := Vec2(149 + 150*Math.Cos(theta), 149 + 150*Math.Sin(theta)), this.Position := this.Start.Clone(), this.Angle := theta, this.Trigger := Math.ToRadians(trigger)
